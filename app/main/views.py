...
def followers(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.index'))
    page = request.args.get('page', 1, type=int)
    pagnation = user.follower.paginate(
...
def show_all():
    ...
    resp.set_cookies('show_followed', '', max_page=30*24*60*60)
    return resp
...
def show_followed():
    ...
    resp.set_cookies('show_followed', '1', max_page=30*24*60*60)
    return resp
...
@main.route('/moderate')
@login_required
@permission_required(Permisson.MODERATE_COMMETS)
def moderate():
    page = request.args.get('page', 1, type=int)
...
@login_required
@permission_required(Permission.MOMODERATE_COMMENTS)
